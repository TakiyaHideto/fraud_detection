...
public class AccumulateDomain {
    ...
    public static class AccumulateReducer extends Reducer<Text, Text, NullWritable, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values,Context context)
            ...
            HashMap<String,HashSet<String>> domainTimeMap = new HashMap<String,HashSet<String>>();
            ...
            for (Text value: values){
                ...
                String[] domainList = elements[2].split(Properties.Base.CTRL_B,-1);
                for (String domainInfo: domainList){
                    String domain = domainInfo.split(Properties.Base.CTRL_C,-1)[0];
                    String timestamp = domainInfo.split(Properties.Base.CTRL_C,-1)[1];
                    String date = "";
                    String hour = "";
                    try{
                    ...
                    if (domainTimeMap.size()<1000) {
                        CommUtil.addInfoToMapWithSet(domainTimeMap, domain, date + Properties.Base.CTRL_D + hour);
                        CommUtil.addInfoToMap(timeFreqMap, date + Properties.Base.CTRL_D + hour);
                    }
                }
            }
            String domainInfoString = "";
            for (String domain: domainTimeMap.keySet()){
                String timeString = "";
                for (String time: domainTimeMap.get(domain)){
                    timeString = time + Properties.Base.CTRL_D + String.valueOf(timeFreqMap.get(time));
                    domainInfoString += domain + Properties.Base.CTRL_C + timeString + Properties.Base.CTRL_B;
                }
            }
            domainInfoString = domainInfoString.substring(0,domainInfoString.length()-1);

            value_result.set(key.toString() + Properties.Base.CTRL_A + domainInfoString);
    ...
    public static void main(String[] args) throws Exception{
        ...
        Job job = new Job(conf,"AccumulateDomain");
        conf.setBoolean("mapred.reduce.tasks.speculative.execution", false);

        job.getConfiguration().setBoolean("mapred.compress.map.output", true);
        job.getConfiguration().set("mapred.map.output.compression.codec", "org.apache.hadoop.io.compress.GzdomainCodec");
        job.getConfiguration().setBoolean("mapred.output.compress", true);
        job.getConfiguration().set("mapred.output.compression.codec", "org.apache.hadoop.io.compress.GzdomainCodec");

        FileSystem fs = FileSystem.get(conf);

        job.setJarByClass(AccumulateDomain.class);
        ...
        for (int i=0;i<3;i++) {
            CommUtil.addInputFileComm(job, fs, bidLog + "/" + currentData + "/", TextInputFormat.class, AccumulateMapper.class);
